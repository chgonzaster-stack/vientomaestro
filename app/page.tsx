        ...
        <CardHeader>
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <h2 className="text-base opacity-90 font-medium">Transpositor de música para instrumentos de viento</h2>
            </div>
            <div className="w-44">
        ...
        <CardContent>
            ...
            <TabsContent value="instrumento">
              <div className="grid gap-3 md:grid-cols-2 mt-4">
                <div>
                  ...
                  <Select value={originInstrument?.value ?? ''} onValueChange={(v)=>setOriginInstrument(INSTRUMENTS_DATA.find(i=>i.value===v) ?? null)}>
                    <SelectTrigger><SelectValue placeholder="Seleccionar origen..." /></SelectTrigger>
                    <SelectContent>
                      {INSTRUMENTS_DATA.map(i => <SelectItem key={i.value} value={i.value}>{i.label}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div>
                  <label className="text-sm font-medium">Instrumento de Destino</label>
                  <Select value={targetInstrument?.value ?? ''} onValueChange={(v)=>setTargetInstrument(INSTRUMENTS_DATA.find(i=>i.value===v) ?? null)}>
            ...
            <TabsContent value="tono">
              <div className="grid gap-3 md:grid-cols-2 mt-4">
                <div>
                  ...
                  <Select value={originKey?.name ?? ''} onValueChange={(v)=>setOriginKey(concertKeys.find(k=>k.name===v) ?? null)}>
                    <SelectTrigger><SelectValue placeholder="Seleccionar tono..." /></SelectTrigger>
                    <SelectContent>
                      {concertKeys.map(k => <SelectItem key={k.name} value={k.name}>{k.name}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div>
                  <label className="text-sm font-medium">Tono Deseado</label>
                  <Select value={targetKey?.name ?? ''} onValueChange={(v)=>setTargetKey(concertKeys.find(k=>k.name===v) ?? null)}>
                    <SelectTrigger><SelectValue placeholder="Seleccionar tono..." /></SelectTrigger>
                    <SelectContent>
                      {concertKeys.map(k => <SelectItem key={k.name} value={k.name}>{k.name}</SelectItem>)}
                    </SelectContent>
          ...
          <div className="grid gap-4 md:grid-cols-2 mt-6">
            <div>
              ...
              <Textarea placeholder="Ej: C G Am F / Bb Eb Cm F7… o cargue un archivo .txt" value={inputText} onChange={e=>setInputText(e.target.value)} className="min-h-[220px]" />
